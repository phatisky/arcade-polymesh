...
namespace Polymesh {
    ...
    export function render(mymesh: mesh, image: Image, innered?: boolean) {
        function update() {
            ...
            let angleX = ax, angleY = ay, angleZ = az
            ...
            function partition(arr: any[], low: number, high: number, rotatedVertices: any[], pivotIndex: number): number {
                ...
                for (let j = low; j <= high; j++) {
                    ...
                    if (currentAverageZ > pivotValue) { i++;
            ...
            function quicksort2(triangles: any[], low: number, high: number, rotatedVertices: any[]) {
                while (low < high) {
                    ...
                    if (pi - low < high - pi) { quicksort2(triangles, low, pi - 1, rotatedVertices);
                        low = pi + 1; } else { quicksort2(triangles, pi + 1, high, rotatedVertices);
                        high = pi - 1; }
                }
            }
