...
namespace Polymesh {
    ...
    export function __mesh_del(msh: polymesh) { Polymesh.__mesh[msh.kind][msh.kind_idx] = null; }
    export function __mesh_upd(msh: polymesh) { Polymesh.__mesh[msh.kind][msh.kind_idx] = msh; }
    ...
    export function meshAll(kind?: number) {
        return Polymesh.__mesh[kind].filter( msh => msh != null || !msh.isDel())
    }
